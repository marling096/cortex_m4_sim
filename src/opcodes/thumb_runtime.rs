...
pub fn scan_block(
    ...
    while instruction_count < max_instructions && current_pc < end_pc_exclusive {
        let info = inspect_block_boundary(cpu, current_pc)?;
        last_pc = current_pc;
        instruction_count += 1;
        ...
        if info.is_it {
            pending_it_following = info.it_following_count;
        }

        let it_block_ends_here = was_inside_it_block && pending_it_following == 0;
        let next_pc = current_pc.wrapping_add(info.size);
        if info.terminates_block || it_block_ends_here || next_pc >= end_pc_exclusive {
    ...
    if instruction_count == 0 {
        return Err(format!(
            "failed to scan block at PC 0x{start_pc:08X}: outside code segment"
        ));
    }
...
fn inspect_block_boundary(cpu: &Cpu, current_pc: u32) -> Result<BlockBoundaryInfo, String> {
    let hw1 = fetch16(cpu, current_pc);
    if is_thumb32(hw1) {
        let hw2 = fetch16(cpu, current_pc.wrapping_add(2));
        inspect_block_boundary_32(hw1, hw2, current_pc)
    } else {
        inspect_block_boundary_16(hw1, current_pc)
    }
}

fn inspect_block_boundary_16(hw: u16, _current_pc: u32) -> Result<BlockBoundaryInfo, String> {
    if (hw & 0xF800) == 0xE000 {
        return Ok(BlockBoundaryInfo { size: 2, terminates_block: true, is_it: false, it_following_count: 0 });
    }
    ...
    if (hw & 0xFE00) == 0xBC00 {
        ...
        return Ok(BlockBoundaryInfo { size: 2, terminates_block: extra_pc, is_it: false, it_following_count: 0 });
    }
    ...
    if (hw & 0xFC00) == 0x4400 {
        ...
        let terminates_block = match op {
            2 => rdn == 15,
            3 => true,
            _ => false,
        };
        return Ok(BlockBoundaryInfo { size: 2, terminates_block, is_it: false, it_following_count: 0 });
    }

    if (hw & 0xFF00) == 0xBF00 && (hw & 0x000F) != 0 {
        let it_mask = (hw & 0xF) as u8;
        let following = (4u32.saturating_sub(it_mask.trailing_zeros())) as usize;
        return Ok(BlockBoundaryInfo { size: 2, terminates_block: false, is_it: true, it_following_count: following });
    }

    Ok(BlockBoundaryInfo { size: 2, terminates_block: false, is_it: false, it_following_count: 0 })
}

fn inspect_block_boundary_32(hw1: u16, hw2: u16, current_pc: u32) -> Result<BlockBoundaryInfo, String> {
    let _ = current_pc;
    if (hw1 & 0xF800) == 0xF000 && (hw2 & 0xD000) == 0xD000 {
        return Ok(BlockBoundaryInfo { size: 4, terminates_block: true, is_it: false, it_following_count: 0 });
    }

    if hw1 == 0xE8BD {
        let wrote_pc = (multi_reg_mask_pop(hw2) & (1 << 15)) != 0;
        return Ok(BlockBoundaryInfo { size: 4, terminates_block: wrote_pc, is_it: false, it_following_count: 0 });
    }

    if (hw1 & 0xFFD0) == 0xE890 {
        let wrote_pc = (hw2 & (1 << 15)) != 0;
        return Ok(BlockBoundaryInfo { size: 4, terminates_block: wrote_pc, is_it: false, it_following_count: 0 });
    }

    if hw1 == 0xF8DF {
        let rt = ((hw2 >> 12) & 0xF) as u32;
        return Ok(BlockBoundaryInfo { size: 4, terminates_block: rt == 15, is_it: false, it_following_count: 0 });
    }

    if (hw1 & 0xFBF0) == 0xF240 && (hw2 & 0x8000) == 0 {
        let rd = ((hw2 >> 8) & 0xF) as u32;
        return Ok(BlockBoundaryInfo { size: 4, terminates_block: rd == 15, is_it: false, it_following_count: 0 });
    }
    ...
    if (hw1 & 0xFA00) == 0xF000 && (hw2 & 0x8000) == 0 {
        ...
        let rd = ((hw2 >> 8) & 0xF) as u32;
        let terminates_block = matches!(op, 0 | 1 | 2 | 8) && (rd == 15 || (op == 2 && rn == 15 && rd == 15));
        return Ok(BlockBoundaryInfo { size: 4, terminates_block, is_it: false, it_following_count: 0 });
    }

    if (hw1 & 0xFE00) == 0xEA00 || (hw1 & 0xFE00) == 0xEB00 {
        let op = (hw1 >> 5) & 0xF;
        let rd = ((hw2 >> 8) & 0xF) as u32;
        let terminates_block = matches!(op, 0 | 2 | 8) && rd == 15;
        return Ok(BlockBoundaryInfo { size: 4, terminates_block, is_it: false, it_following_count: 0 });
    }
    ...
    Ok(BlockBoundaryInfo { size: 4, terminates_block: false, is_it: false, it_following_count: 0 })
}
